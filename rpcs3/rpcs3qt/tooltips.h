...
public:
	...
	const struct settings
	{
		...
		const QString output_scaling_mode = tr("Final image filtering. Nearest applies no filtering, Bilinear smooths the image, and FidelityFX Super Resolution 1 enhances upscaled images.\nNVIDIA DLSS Super Resolution is an experimental Vulkan-only path that requires an RTX GPU and a build configured with the NVIDIA DLSS SDK. It uses the bound depth buffer plus NVIDIA Optical Flow motion estimation, and falls back to bilinear scaling when unavailable.\nFidelityFX and DLSS do not work with stereo 3D output for now.");
