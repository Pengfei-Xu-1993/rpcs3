...
- 同时索引 Resource Studio 档案中的完整 mip 链和“省略最后一级档案尾 mip”的运行时视图。《升天》实际交给 RSX 的多数纹理属于后者；命中后只上传游戏可见的 mip 前缀，多余的档案尾 mip 不参与上传。
...
- 对任何缺失、损坏、格式变化、运行时所需 mip 不足或非整数缩放自动回退原纹理。

索引或 DDS 改动后应重启 RPCS3。当前版本不做运行中热重载。

## 已验证状态

2026-08-16 使用隔离配置、原版 `MP-RESOURCES.PSARC`/`RESOURCES.PSARC` 和完整 1311 项候选包验证：

- 130/130 本地测试通过，并完成 Vulkan 与主程序增量构建。
- 自动进入真实 3D 场景后，24 个不同颜色纹理发生 692 次 DDS 加载；最新运行计数为 689 次匹配、689 次 Vulkan 上传、0 次上传失败。
- 实际命中 7 个资源组，包括 `R_KRATOSOLIVEWT00`、`R_ALECTO00`、`R_MEDUSA10` 和多个 `R_PARASITE*`；未发现访问冲突、客体崩溃、`VK_ERROR_DEVICE_LOST` 或不兼容 DDS。
- 法线候选仍保持默认关闭。本轮证据只证明普通颜色贴图挂载链已经真实工作，不把未经校验的法线高清化结果当成完成项。
